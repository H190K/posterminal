  ...
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/login") {
      ...
      if (formData.get("password") === env.TERMINAL_PASSWORD) {
        return new Response("Logged In", { status: 302, headers: { "Location": "/", "Set-Cookie": `session=${env.TERMINAL_PASSWORD}; HttpOnly; Secure; SameSite=Strict; Max-Age=120` }});
      }
    ...
    try {
      ...
      if (url.pathname === "/pay") {
        ...
        const spResponse = await fetch("https://sindipay.com/api/v1/payments/gateway/", {
          method: "POST",
          headers: { "X-API-Key": env.API_KEY, "Content-Type": "application/json", "Accept": "application/json", "User-Agent": `${env.MERCHANT_NAME || "Generic"}-POS/1.0`, "Referer": "https://sindipay.com/" },
          body: JSON.stringify({ title: `${env.MERCHANT_NAME || "POS"} Payment`, order_id: `POS-${Date.now()}`, total_amount: amount, currency: "IQD", customer_name: name, customer_email: email, callback_url: successUrl, webhook_url: secureWebhookUrl })
        });
        ...
        if (timestamp) {
          try {
            ...
            if (!isNaN(date.getTime())) {
              timeStr = date.toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true, timeZone: 'Asia/Baghdad' });
            }
          } catch (e) {
            const now = new Date();
            timeStr = now.toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true, timeZone: 'Asia/Baghdad' });
          }
        } else {
          const now = new Date();
          timeStr = now.toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true, timeZone: 'Asia/Baghdad' });
